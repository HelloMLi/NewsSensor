...
public class HotProvinceService {

	private static final String tag = "FocusService";
	private DBopenHelper dbopenHelper;

	public HotProvinceService(Context context) {
		super();
		this.dbopenHelper = new DBopenHelper(context);
	}
	public HotProvinceService() {
	...
	public void save(Topic topic)
	{
		...
		db.execSQL("insert into hotprovinces(id,name,value) values(?,?,?)",
...
//	}
	...
	{
		SQLiteDatabase db=dbopenHelper.getWritableDatabase();
		db.execSQL("update hotprovinces set name=?,value=? where id=?",
	...
	public Topic find(int id)
	{
		...
		Cursor cursor=db.rawQuery("select * from hotprovinces where id=?", new String[]{new Integer(id).toString()});
	...
	public int getcount()
	{
		Log.i(tag, "获取关注数目");
		SQLiteDatabase db=dbopenHelper.getReadableDatabase();
		Cursor cursor=db.rawQuery("select count(*)  from hotprovinces ", null);
		cursor.moveToFirst();
